...
// 공지 메시지
$notice_begin_time = "20190519" . "000000";
$notice_end_time = "20190519" . "100000";

$notice_message = "";

if ( (current_datetime() >= $notice_begin_time) && (current_datetime() <= $notice_end_time) ) 
{
    $notice_message = '
        <strong>
            CJ대한통운 택배정보시스템 정기점검 작업 안내<br />
            <br />
            CJ대한통운 택배 홈페이지를 이용해주시는 고객님께 감사드립니다.<br />
            아래의 일정으로 CJ대한통운 택배정보시스템의 정기점검 및 DB성능 개선작업을 실시합니다.<br />
            <br />
            일시 : 5월 19일(일요일) 00시 ~ 11시<br />
            <br />
            정기점검 동안 배송조회 등 홈페이지와 택배 앱을 이용한 서비스가 일시 중단되오니 이용에 참고하시기 바랍니다. 감사합니다.<br />
            <br />
        </strong>
    ';
}
tp_set("notice_message", $notice_message);
...
switch ($rand_value) {
    ...
    case 3:
        ...
        // 배송조회 DB파일 정리 (일자 기준)
        set_deliverytracking_db_auto_delete_by_date();
        
        break;

    case 4:
        // 다중검색 DB파일 정리 (항목수 기준)
        set_multisearch_db_auto_delete();
        
        break;

    case 5:
        // 배송조회 자동 증가값 초기화
        set_deliverytracking_db_auto_increment_init();
