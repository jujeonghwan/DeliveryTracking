...
function get_deliverytracking_result($deliverytype, $invoice_no) {
    ...
    if ($deliverytype == $COMMON_DELIVERYTYPE_ARRAY["UPS(국제)"]) {
        // echo "UPS(국제)"; exit;
        return get_deliverytracking_result_ups($deliverytype, $invoice_no);
    }
    */
    ...
    switch ($deliverytype) {
        ...
        case $COMMON_DELIVERYTYPE_ARRAY["TNT(국제)"]: 
            ...
            */
            ...
            // 선처리 Referer
            if ($deliverytype == $COMMON_DELIVERYTYPE_ARRAY["합동택배"]) {
                $curlopt_referer    =   "https://hdexp.co.kr/basic_delivery.hd?barcode=" . $invoice_no;
            }
    ...
    */
    ...
    if ($deliverytype == $COMMON_DELIVERYTYPE_ARRAY["UPS(국제)"]) {
        ...
        // ex) [Locale] => ko_KR [TrackingNumber] => Array ( [0] => 1Z31X7610476007308 )
        $ups_data_array = array (
            "Locale" => "ko_KR",
        ...
        $request_uri_query = json_encode($ups_data_array);
    ...
    */
    ...
    if ($deliverytype == $COMMON_DELIVERYTYPE_ARRAY["CU편의점택배"]) {
        ...
        $curlopt_referer    =   "https://www.fedex.com/apps/fedextrack/index.html?tracknumbers=" . $invoice_no . "&cntry_code=kr";
    }
    else if ($deliverytype == $COMMON_DELIVERYTYPE_ARRAY["합동택배"]) {
        // $curlopt_referer =   "http://hdexp.co.kr/deliverySearch2.hd?barcode=" . $invoice_no;
        $curlopt_referer    =   "https://hdexp.co.kr/basic_delivery.hd?barcode=" . $invoice_no;
    }
    else if ($deliverytype == $COMMON_DELIVERYTYPE_ARRAY["매일택배(캐나다->한국)"]) {
        $curlopt_referer    =   "http://www.mailexglobal.com/cs-detail3.html";
    }
        ...
        $curlopt_referer    =   "https://www.ups.com/";
    }
    else {
        $curlopt_referer    =   $curlopt_url;    
    }
    ...
    if (    ($deliverytype == $COMMON_DELIVERYTYPE_ARRAY["쿠팡로켓배송"]) ||
        ...
        if ($COMMON_DELIVERYMETHOD_ARRAY[$deliverytype] == "POST") {
            ...
            if ($deliverytype == $COMMON_DELIVERYTYPE_ARRAY["FedEx(국제)"]) {
                $string .= "Referer: https://www.fedex.com/apps/fedextrack/index.html?tracknumbers=" . $invoice_no . "&cntry_code=kr\r\n";
                $string .= "Host: www.fedex.com\r\n";
                $string .= "Cookie: siteDC=edc; fdx_cbid=31382261241557680220421120003361; fdx_locale=ko_KR; bm_sz=9677A723843686F25B8C223F6ACB7908~YAAQnfNUuHzvO1hqAQAAcuj5rAOI9rYnH/N2VPV5H+yk15L2qmfo3MmlfRla7B9/mbtCZ9sL9dDat6XpYLP8N0LtCknAhSPoZsVIw6aj68jP/gYJAGXlWwYdtVmqkkyj3J4/Ic5DbW2dFado9wWQ9pOu2DNxbqFwUq7KM+9W4hyhMwgfPppQ9nyizQdhnsQ=; _abck=0B7E774AA5841A5341CAA6673B0FC383B854F39D370300005C50D85CD35DE824~0~N2zPylm9q+rCJfzfrWZeiq63jd+LqqPTzYkWQgm8MCU=~-1~-1; AMCVS_1E22171B520E93BF0A490D44%40AdobeOrg=1; ak_bmsc=597DBEC94E1B69A4B5AEF9389282971BB854F39D370300005C50D85CDE4D8D7F~; Nina-nina-fedex-session=%7B%22locale%22%3A%22ko_kr%22%2C%22lcstat%22%3Afalse%7D; tracking_locale=ko_KR; fdx_redirect=ko-kr; isTablet=false; isMobile=false; isWireless=false; level=test; check=true; _gcl_au=1.1.341143720.1557680542; s_cc=true; s_sq=fedexglbl%252Cfedexapac%3D%2526c.%2526a.%2526activitymap.%2526page%253Dfedex%25252Ftracking%2526link%253D%2525EB%2525B0%2525B0%2525EC%252586%2525A1%252520%2525EC%2525A1%2525B0%2525ED%25259A%25258C%2526region%253Dnumber%2526pageIDType%253D1%2526.activitymap%2526.a%2526.c%2526pid%253Dfedex%25252Ftracking%2526pidt%253D1%2526oid%253D%2525EB%2525B0%2525B0%2525EC%252586%2525A1%252520%2525EC%2525A1%2525B0%2525ED%25259A%25258C%2526oidt%253D3%2526ot%253DSUBMIT; mbox=session#1557680222955-215043#1557683879|PC#1557680222955-215043.17_210#1558891619; s_pers=%20s_vnum%3D1557720000193%2526vn%253D1%7C1557720000193%3B%20s_nr%3D1557681995943-New%7C1589217995943%3B%20s_tbm%3D1%7C1557683795966%3B%20gpv_pageName%3Dfedex%252Ftracking%7C1557683817403%3B%20s_invisit%3Dtrue%7C1557683817476%3B%20s_dfa%3Dfedexapac%252Cfedexglbl%7C1557683818751%3B; s_sess=%20s_cc%3Dtrue%3B%20s_ppv%3Dkr%252Fko%252Ffedex%252Funified%252Ftrackdetailspage%252C100%252C87%252C1645%3B%20s_visit%3D1%3B%20SC_LINKS%3D%3B%20setLink%3D%3B; AMCV_1E22171B520E93BF0A490D44%40AdobeOrg=817868104%7CMCIDTS%7C18029%7CMCMID%7C54982048504775314811062127540986641486%7CMCAAMLH-1558286818%7C7%7CMCAAMB-1558286818%7CRKhpRz8krg2tLO6pguXWp5olkAcUniQYPHaMWWgdJ3xzPWQmdj0y%7CMCOPTOUT-1557689218s%7CNONE%7CMCAID%7CNONE%7CvVersion%7C2.3.0; bm_sv=924049C0D0610D6ED3D48C6FD680BA77~2Z9JqweZ11j3H0f8AFi/Fyw4uacy8wOVQedOB69HNN2VopcPceRdWvuDPZqnZGzPZ6I3luX9+5NJjzJQZHaJKiAUpZj6ffab2QgDJTRTTH63/vaEnpu910qVM+8iX3dg49gAQOw6Cs8ErI+jZW81QjBmw9kHkk0wwnkC7uPgEsE=\r\n";
            }
            */
...
//            if ($deliverytype == $COMMON_DELIVERYTYPE_ARRAY["합동택배"]) {
//                $string = $COMMON_DELIVERYMETHOD_ARRAY[$deliverytype] . " " . $request_uri . " //HTTP/1.1\r\n";    
//                $string = "GET https://hdexp.co.kr/deliverySearch2.hd?barcode=3132062290287 //HTTP/1.1\r\n";
//                
...
//                $string .= "Referer: https://hdexp.co.kr/delivery_search.hd\r\n";
//                $string .= "Accept-Encoding: gzip, deflate, br\r\n";
//                $string .= "Accept-Language: en-US,en;q=0.9,ko;q=0.8,la;q=0.7\r\n";
//                $string .= "Cookie: JSESSIONID=5693B05FE79DADA3E8E1957F23A19E61\r\n";  
//
//                $string .= "Host: hdexp.co.kr\r\n";
//                $string .= "Connection: keep-alive\r\n\r\n";           
//            }
        }
        ////////////////////////////////////////////////////////////////////////////////
    ...
    */
    ...
    if ($deliverytype == $COMMON_DELIVERYTYPE_ARRAY["롯데택배"]) {
        $text = "
            ...
            width=\"550px\" height=\"500px\" seamless=\"seamless\" frameBorder=\"0\" scrolling=\"no\"></iframe>
        ";
    ...
    ////////////////////////////////////////////////////////////////////////////////
    // 합동택배의 경우에는 임시로 <iframe>으로 처리 (2019.06.01)
    if ($deliverytype == $COMMON_DELIVERYTYPE_ARRAY["합동택배"]) {
        $temp_datetime = current_datetime();                // ex) 20190603105630

        $text = "
            <input type=\"button\" class=\"btn btn-success\" value=\"합동택배 처리현황 상세보기 (Click)\" onclick='document.getElementById(\"hdexp_frame_" . $temp_datetime . "\").src=\"https://hdexp.co.kr/basic_delivery.hd?barcode=" . $invoice_no . "\";' /><br /><br />
            <iframe id=\"hdexp_frame_" . $temp_datetime . "\" src=\"about:blank\" 
            width=\"800px\" height=\"550px\" seamless=\"seamless\" frameBorder=\"0\" scrolling=\"no\"></iframe>
        ";
    ...
    switch ($deliverytype) {
        ...
        case $COMMON_DELIVERYTYPE_ARRAY["롯데글로벌로지스(국제)"]:            
            $result_deliverytracking = get_result_deliverytracking_lotteglogis_i($content_temp);            
            break;  
...
// 배송조회 DB파일 정리 (일자 기준)
function set_deliverytracking_db_auto_delete_by_date() {
    global $SITE_VAR;

    // 첫번째 존재하는 데렉터리를 구한다.    
    $delete_date = date_difference_day(current_date(), -$SITE_VAR["deliverytracking_db_save_date"]);
    // $delete_date = date_difference_day(current_date(), -5);

    // 삭제 대상 배송조회(deliverytracking_tb) 항목수를 구한다.
    $query = "select ";
    $query .= "count(*) as deliverytracking_count ";
    $query .= "from deliverytracking_tb ";
    $query .= "where dt_regtime < '" . $delete_date . "000000' ";
    // echo "<br /><br />" . $query; exit;

    $result_count = db_query($query);
    $row_count = db_fetch_array($result_count);

    if ($row_count["deliverytracking_count"] <= 0) {
        return true;
    }
...
// 배송조회 자동 증가값 초기화
function set_deliverytracking_db_auto_increment_init() {
    global $SITE_VAR;
    ...
    while ($row_reset = db_fetch_array($result_reset)) {
        ...
        if ($dt_id_old != $dt_id_new) {
            $update_count++;            // ID를 변경한 갯수 증가
            ...
            // echo "<br />dt_id_old: " . $dt_id_old . " -> dt_id_new:";
            // /*
            if ($update_count == 1) {
                echo "
                    <div class=\"container text-center\">   
                        <p class=\"text-muted\">" . $dt_id_new . "</p>
                    </div>
                ";    
            }            
            // */
    ...
    // echo "<br />update_count: ". $update_count; exit;
